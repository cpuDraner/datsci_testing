...
class HAR:
    def __init__(self, path: str, n_people: int=9999):
        ...
        scs=self._read_steps(path,n_people)
        self.df = self._combine(hrs, mots, lbls, scs)
    ...
    def _read_steps(self, path: str, n_people: int = 1):
        """read the steps from the base path."""
        ...
        for person in people:
            hrdf = pd.read_csv(
                os.path.join(path, person), names=["timestamp", "sc"]
            ).drop_duplicates("timestamp", keep="first")
            hrdf["person"] = count
            dfs.append(hrdf)
            count = count + 1  # count += 1
    ...
    def _combine(self, hrs: pd.DataFrame, mots: pd.DataFrame, lbls: pd.DataFrame, scs:pd.DataFrame):
        ...
        scs_interp = (
            dfs["scs"]
            .reindex(dfs["scs"].index.union(shared_index))
            .interpolate(method="index")
            .loc[shared_index]
        )
